...
class AbstractiveSumRobertaDataset(FairseqDataset):
    """
    ...
    def __init__(
        ...
        pretrained_task=None, mask_ratio=None, max_mask_len=None, min_mask_len=None, ensemble_pretrain=False,
    ):
        ...
        self.ensemble_pretrain = ensemble_pretrain

    def __getitem__(self, index):
        if not self.ensemble_pretrain:
            ...
            if p == 0:  # 1/3
                src_item, tgt_item = self.get_nsp_pair(index)
            elif p == 1:  # 1/3
                src_item, tgt_item = self.get_mask_span_pair(index)
            else:  # 1/3
                src_item, tgt_item = self.get_mix_order_pair(index)
    ...
    def get_mask_span_pair(self, index):
        src_tensor = self.src[index]
        src_list = src_tensor.tolist()  # [self.src_dict.eos_index] + src_item.tolist()
        # print()
        # org_src = [self.src_dict[word] for word in src_list]
        ...
        def create_src(src_len):
            ...
            for i in range(last_sent_len):
                if i == 0:
                    ...
                    doc.append(self.src_dict.unk())
            doc[-2] = self.src_dict.sep()
            doc[-1] = self.src_dict.eos()
            # print('src', len(doc))
        ...
        def create_tgt(tgt_len):
            ...
            for i in range(last_sent_len):
                if i == 0:
                    summary.append(self.tgt_dict.sep())
                summary.append(self.tgt_dict.unk())
            summary[-1] = self.tgt_dict.eos()
            # print('tgt', len(summary))
            return torch.LongTensor(summary)

        return self.collater([
            {
                'id': i,
                'source': create_src(src_len),  # self.src_dict.dummy_sentence(src_len),
                'target': create_tgt(tgt_len),  # self.tgt_dict.dummy_sentence(tgt_len) if self.tgt_dict is not None else None,
            }
            for i in range(bsz)
        ])
