...
class Trainer(object):
    ...
    def __init__(self, args, task, model, criterion, dummy_batch, oom_batch=None):
        ...
        if self.args.sep_optim:
            self._dec_optim_history = None
            self._dec_optimizer = None
            self._dec_lr_scheduler = None
            self._dec_prev_grad_norm = None
    ...
    def init_meters(self, args):
        ...
        if args.sep_optim:
            self.meters['dec_gnorm'] = AverageMeter()  # gradient norm for decoder
    ...
    def dec_optimizer(self):
        if self.args.sep_optim:
            if self._dec_optimizer is None:
                self._build_optimizer()
    ...
    @property
    def dec_lr_scheduler(self):
        if self.args.sep_optim:
            if self._dec_lr_scheduler is None:
                self._build_optimizer()  # this will initialize self._lr_scheduler
            return self._dec_lr_scheduler
        else:
            return None
    ...
    def _build_optimizer(self):
        ...
        if self.args.sep_optim:
            ...
            self._dec_lr_scheduler = lr_scheduler.build_lr_scheduler(self.args, self._dec_optimizer, decoder=True)
        else:
            self._lr_scheduler = lr_scheduler.build_lr_scheduler(self.args, self.optimizer)
    ...
    def load_checkpoint(self, filename, reset_optimizer=False, reset_lr_scheduler=False, optimizer_overrides=None):
        ...
        if not self.args.sep_optim:
            ...
            extra_state, self._optim_history, self._dec_optim_history, \
            last_optim_state, last_dec_optim_state = utils.load_model_state_for_sep_optim(
                filename, self.get_model(),
            )
        if last_optim_state is not None and not reset_optimizer:
            ...
            if self.args.sep_optim:
                last_dec_optim = self._dec_optim_history[-1]
                assert last_dec_optim['criterion_name'] == self.criterion.__class__.__name__, \
                    'criterion does not match; please reset the optimizer (--reset-optimizer)'
                assert last_dec_optim['dec_optimizer_name'] == self.dec_optimizer.__class__.__name__, \
                    'optimizer does not match; please reset the optimizer (--reset-optimizer)'

            if not reset_lr_scheduler:
                self.lr_scheduler.load_state_dict(last_optim['lr_scheduler_state'])
                if self.args.sep_optim:
                    self.dec_lr_scheduler.load_state_dict(last_dec_optim['dec_lr_scheduler_state'])
            self.optimizer.load_state_dict(last_optim_state, optimizer_overrides)
            if self.args.sep_optim:
                self.dec_optimizer.load_state_dict(last_dec_optim_state, optimizer_overrides)
    ...
    def train_step(self, samples, dummy_batch=False):
        ...
        try:
            ...
            if self.args.sep_optim:
                self.dec_optimizer.multiply_grads(self.args.distributed_world_size / float(sample_size))
            # clip grads
            grad_norm = self.optimizer.clip_grad_norm(self.args.clip_norm)
            self._prev_grad_norm = grad_norm
            if self.args.sep_optim:
                dec_grad_norm = self.dec_optimizer.clip_grad_norm(self.args.clip_norm)
                self._dec_prev_grad_norm = dec_grad_norm
            ...
            if self.args.sep_optim:
                self.dec_lr_scheduler.step_update(self._num_updates)
            ...
            if self.args.sep_optim:
                self.meters['dec_gnorm'].update(dec_grad_norm)
    ...
    def dec_lr_step(self, epoch, val_loss=None):
        """Adjust the decoder learning rate based on the validation loss."""
        return self.dec_lr_scheduler.step(epoch, val_loss)

    def dec_lr_step_update(self, num_updates):
        """Update the learning rate after each update."""
        return self.dec_lr_scheduler.step_update(num_updates)

    def get_dec_lr(self):
        """Get the current learning rate."""
        return self.dec_optimizer.get_lr()
