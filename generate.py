...
#     assert args.save_path is not None, '--save-path required for generation!'
#     pt_path = args.path
#     save_path = args.save_path
#     filename = os.path.basename(pt_path)
#     epoch = filename[10:-3]
#     output_file = 'epoch.{0}.{1}.txt'.format(epoch, args.gen_subset)
#     output_file_path = os.path.join(save_path, output_file)
#     if os.path.isfile(output_file_path):
#         with open(output_file_path, 'r') as reader:
#             if len(reader.readlines()) > 0:
#                 line = reader.readlines()[-1].strip()
#                 key = 'Generate {0} with beam'.format(args.gen_subset)
#                 if key in line:
#                     return True
#     return False
                ...
                if not args.quiet:
                    if src_dict is not None:
                        if not args.isRoberta:
                            ...
                            src_out = tokenizer.convert_tokens_to_string(src_text)
                    ...
                    if has_target:
                        if not args.isRoberta:
                            ...
                            tgt_out = tokenizer.convert_tokens_to_string(tgt_text)
                ...
                for i, hypo in enumerate(hypos[i][:min(len(hypos), args.nbest)]):
                    ...
                    if not args.quiet:
                        if not args.isRoberta:
                            ...
                            hypo_out = tokenizer.convert_tokens_to_string(hypo_text)
                            print('H-{}\t{}\t{}'.format(sample_id, hypo['score'], hypo_out))
                        print('P-{}\t{}'.format(
                            sample_id,
                            ' '.join(map(
