...
def main(args, init_distributed=False):
    ...
    dummy_batch = task.dataset('train').get_dummy_batch(args.max_tokens, max_positions, batch_size=args.max_sentences)
    ...
    if not load_checkpoint(args, trainer, epoch_itr):
        if args.task == 'abstractive_summarization_bert' or args.task == 'abstractive_summarization_roberta':
            ...
            elif hasattr(args, 'roberta_decoder') and args.roberta_decoder and hasattr(args, 'roberta_decoder_initialization') and args.roberta_decoder_initialization:
...
def train(args, trainer, task, epoch_itr):
    ...
    # print(trainer.get_model().encoder.roberta.encoder.layer[11].output.LayerNorm.weight.data)
    # print(trainer.get_model().decoder.layers[11].output.LayerNorm.weight.data)
    ...
    for i, samples in enumerate(progress, start=epoch_itr.iterations_in_epoch):
        ...
        # log mid-epoch stats
        stats = get_training_stats(trainer, sep_optim=args.sep_optim)
    ...
    for k in [
        'train_loss', 'train_nll_loss', 'wps', 'ups', 'wpb', 'bsz', 'gnorm', 'dec_gnorm', 'clip',
    ]:
...
def get_training_stats(trainer, sep_optim=False):
    ...
    if sep_optim:
        stats['dec_lr'] = trainer.get_dec_lr()
    stats['gnorm'] = trainer.get_meter('gnorm')
    stats['dec_gnorm'] = trainer.get_meter('dec_gnorm')
...
def load_checkpoint(args, trainer, epoch_itr):
    ...
    if os.path.isfile(checkpoint_path):
        ...
        if extra_state is not None:
            ...
            if args.sep_optim:
                trainer.dec_lr_step(epoch_itr.epoch)
                trainer.dec_lr_step_update(trainer.get_num_updates())
